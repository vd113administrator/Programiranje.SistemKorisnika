<%-- 
    Document   : StranicaGrupe
    Created on : Oct 10, 2017, 2:32:23 PM
    Author     : Mikec
--%>
...
<%
        ...
        if(bean!=null && bean.getUsername().length()!=0){
            ...
            out.println("<center>\n" +
    "                <font color=\"green\">\n" +
    "                    <b>СИСТЕМ КОРИСНИКА<br>СТРАНИЦА ГРУПЕ</b>\n" +  
    "                    <br>Пријављени клијент : "+bean.getUsername()+"\n" +
 ...
 %>
    <script>
        ...
        function odstupanjeOdOpisaGrupe(){
            var text = document.getElementById("ok_data").value;
            text = decodeURI(text); 
            document.getElementById("polje").value = text;
            var domtext = document.createElement('div'); 
            domtext.innerHTML = text; 
            document.getElementById("ispis_opisa_grupe").innerHTML = domtext.innerHTML;
        }
    </script>
    <script>
        var unosOpisaGrupe = false;
        function unosOpisGrupeFF(){
            unosOpisaGrupe = !unosOpisaGrupe;
            if(unosOpisaGrupe){
                document.getElementById('unos_opisa_grupe').style='';
                document.getElementById('ispis_opisa_grupe').style='display: none';
            }else{
                document.getElementById('unos_opisa_grupe').style='display: none';
                document.getElementById('ispis_opisa_grupe').style='';
            }
        }
    </script>
    ...
    <center>
        <%
            if(info.prijavljenAdmin()){
        %>
            <div class='nazad' onclick='unosOpisGrupeFF()'><u>Управљање описом групе</u></div>
        <%
            }else{
        %>
            <div><u>Управљање описом групе</u></div>
        <%
            }
        %>
        <br>
        <form style="display:none" id="unos_opisa_grupe" method="post">
            <font face="Courier New">
                <textarea id="polje" cols="80" rows="25"><%=
                    XMLUtil.getXMLEmbended(opis)
                %></textarea><br>
            </font>
            <input type="button" id="da" value="Прихватање" onclick="potvrdaOpisaGrupe()"/>
            <input type="button" id="ne" value="Одступање" onclick="unosOpisGrupeFF(); odstupanjeOdOpisaGrupe()"/><br>
        </form>
        
        </center>
        
        <div id="ispis_opisa_grupe"><%=
            XMLUtil.getXMLInFirstKeyTag(opis,"<body>","</body>")
        %></div>
    
    <form id="ok_form" method="post" action="/SistemKorisnikaVebAplikacija/PromjenaOpisaGrupe">
